...
namespace OTTER
{
    ...
    public class Sprite
    {
        ...
        public int Heigth
        {
            ...
            {
                if (value <= 0)
                    heigth = GameOptions.SpriteHeight;
                else
                    heigth = value;
            }
        }
        ...
        {
            if (posX + Width >= GameOptions.RightEdge || posX <= GameOptions.LeftEdge)
                return false;
            else
                return true;
        }
        ...
        {
            if (posY + Heigth >= GameOptions.DownEdge || posY <= GameOptions.UpEdge)
                return false;
            else
                return true;
        }        
        ...
        {
            if (this.X + this.Width >= GameOptions.RightEdge)
                return true;
            else
                return false;
        }
        ...
        public bool TouchingBottomEdge()
        {
            if (this.Y + this.Heigth >= GameOptions.DownEdge)
                return true;
            else
                return false;
        }

        public bool TouchingTopEdge()
        {
            if (this.Y <= 0)
                return true;
            else
        ...
        public bool TouchingSprite(Sprite b)
        {
            ...
            Rectangle A = new Rectangle(a.X, a.Y, 400 / 6,132/2);
        ...
        public bool TouchingSprite(Mrkva b)
        {
            Sprite a = this;
        ...
        /// Postavi prozorčić
        /// </summary>
        /// <param name="rows">Broj redaka (sličica u redovima)</param>
        /// <param name="cols">Broj stupaca (sličica u stupcima)</param>
        public void SetTile(int rows, int cols)
        {
            spriteTile = new Tile(CostumeName, width, heigth, rows, cols);
            spriteTile.rowsy = rows;
            spriteTile.colsx = cols;
        }

        /// <summary>
        /// Za pomicanje prozorčića ovisno o smjeru. Trenutna verzija je napravljena samo za lijevo i desno.
        /// </summary>
        /// <param name="left">Indeks retka gdje su sličice za lijevi smjer</param>
        /// <param name="right">Indeks retka gdje su sličice za desni smjer</param>
        public void AnimirajMe(int left, int right)
        {
            //ako nema slike, ignoriraj
            if (spriteTile == null)
                return;

            //ako je samo jedna, ignoriraj
            if (spriteTile.rowsy == 1 && spriteTile.colsx == 1)
                return;

            //ako je smjer desno, postavi trenutni red prozorčića
            if (Direction == 90)
            {
                this.spriteTile.currentRow = right;
            }
            else if (Direction == 270 || Direction == -90)
            {
                this.spriteTile.currentRow = left;
            }

            //pomakni trenutni stupac
            this.spriteTile.currentCol++;

            //ako je došao do kraja, vrati se na početak
            if (this.spriteTile.currentCol >= this.spriteTile.colsx)
            {
                this.spriteTile.currentCol = 0;
            }
        }
        ...
        public void CrtajMe(Graphics g)
        {
            //ako nema sličice, napravi
            if (this.spriteTile == null)
            {
                this.spriteTile = new Tile(this.CostumeName, width, heigth);
            }
            Image newImage = this.spriteTile.image;

            // Napravi pravokutnik za sliku
            Rectangle tileRect = new Rectangle(X, Y, spriteTile.tileWidth, spriteTile.tileHeight);

            spriteTile.SetStart();
            GraphicsUnit units = GraphicsUnit.Pixel;

            // Crtaj na ekran
            g.DrawImage(newImage, tileRect, spriteTile.tileStartX, spriteTile.tileStartY,
                spriteTile.tileWidth, spriteTile.tileHeight, units);
